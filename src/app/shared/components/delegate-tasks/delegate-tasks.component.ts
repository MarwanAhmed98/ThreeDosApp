...
export class DelegateTasksComponent implements OnInit {
    ...
    submissionForm: FormGroup = this.fb.group({
        driveUrl: ['', [
            ...
            (control: any) => {
                ...
                // Google Drive/Docs or GitHub validation
    ...
    onUrlChange(event: Event): void {
        ...
        // Validate URL
        if (url && this.validateSubmissionUrl(url)) {
            // Test file ID extraction
            const fileId = this.extractFileIdFromUrl(url);
            if (fileId) {
                // For GitHub, we don't need to show the ID extraction success toast as the URL itself is the ID
                if (!url.includes('github.com')) {
                    this.toastr.success(`File ID extracted successfully: ${fileId.substring(0, 10)}...`, 'URL Valid');
                }
            } else {
                this.toastr.warning('Valid URL but could not extract ID. Please try a different URL format.', 'URL Warning');
            }
        }
    }
    ...
    private validateSubmissionUrl(url: string): boolean {
        ...
        // Check if it's a Google Drive, Docs, or GitHub URL
        const validPattern = /^https:\/\/(drive\.google\.com|docs\.google\.com|github\.com)/;
        const isValidUrl = validPattern.test(url);

        if (!isValidUrl) {
            this.toastr.warning('Please provide a valid Google Drive, Docs, or GitHub link', 'Invalid URL');
            return false;
        }

        // Check if we can extract a file ID (or valid GitHub ref)
        const fileId = this.extractFileIdFromUrl(url);
        if (!fileId) {
            this.toastr.warning('Unable to validate this URL format.', 'Invalid URL Format');
            return false;
        }
    ...
    extractFileIdFromUrl(url: string): string | null {
        ...
        // For GitHub, we treat the URL itself as the ID to satisfy backend requirements
        if (url.includes('github.com')) {
            return url;
        }
    ...
    submitTask(): void {
        ...
        // Use the form control value directly to ensure it matches what's valid in the form
        const urlRaw = this.submissionForm.get('driveUrl')?.value;
        const url = urlRaw ? String(urlRaw).trim() : '';
        ...
        if (!url || !this.validateSubmissionUrl(url)) {
            this.toastr.error('Please provide a valid Google Drive or GitHub link', 'Invalid URL');
            return;
        }
        ...
        if (!fileId) {
            this.toastr.error(
                'Unable to validate the URL. Please ensure you\'re using a valid link.',
                'Invalid URL Format'
            );
            return;
        }
        ...
        // Ensure the URL is in a shareable format (only for Google Drive)
        const shareableUrl = this.ensureShareableUrl(url);

        const submissionData = {
            task_id: task.id,
            // Explicitly ensuring this is a string to satisfy strict backend validation
            file: String(shareableUrl),
            file_id: String(fileId),
            comment: this.submissionForm.get('comment')?.value || '',
            file_type: this.getFileTypeFromUrl(url)
        };

        // Reverted to using the service now that the service bug is fixed
    ...
    getDriveIcon(url: string): string {
        if (url.includes('github.com')) return 'fab fa-github text-slate-800 dark:text-white';
    ...
    getShareInstructions(): string {
        return `To share your file:
1. Google Drive: Set access to "Anyone with the link"
2. GitHub: Ensure the repository or file is public
